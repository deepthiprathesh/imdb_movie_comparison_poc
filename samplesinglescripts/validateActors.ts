...
async function createExcelFileIfNotExists(filePath: string) {
    ...
    .on('end', async () => {
      ...
      worksheet.eachRow({ includeEmpty: false }, (row, rowNumber) => {
        if (rowNumber > 1) { // Skip header row
          ...
          const actorValidationStatus = row.getCell(2).value as string; // Use "Actor Validation Status" for "Not Processed" batches
      ...
      const validateBatch = async (file: string) => {
        ...
        data.forEach((movie: { Actor_IDs: string; Title: string; }) => {
      ...
      const processBatch = async (file: string) => {
        const row = batchStatusMap.get(file);
        if (row) {
