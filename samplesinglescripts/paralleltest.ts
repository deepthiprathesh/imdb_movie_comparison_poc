...
// working script
const updateBatchStatusInExcel = async (
excelFilePath: string, batchName: string, status: string, reason: string = '', foundDetails: string = '', color?: string  ) => {
...
const compareDataInParallel = async (
    dbData: MovieRecord[],
    excelFilePath: string,
    baseDirectoryPath: string
  ) => {
    ...
    for (let i = 0; i < notProcessedBatches.length; i += maxConcurrentBatches) {
      ...
      const batchSubsetPromises = batchSubset.map((batchName) =>
        compareBatch(dbData, batchName, excelFilePath, baseDirectoryPath)
      );
