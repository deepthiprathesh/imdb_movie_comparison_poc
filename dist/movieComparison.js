...
const updateBatchStatusInExcel = async (excelFilePath, batchName, status) => {
    const workbook = new ExcelJS.Workbook();
    try {
        await workbook.xlsx.readFile(excelFilePath);
    }
...
const loadIMDBBatchFiles = async (directoryPath, batchFiles) => {
    const imdbData = [];
...
const compareData = async (dbData, imdbData, batchName, excelFilePath) => {
    const errors = [];
...
const main = async () => {
    try {
        ...
        for (const chunk of chunkedBatches) {
            await Promise.all(chunk.map(async (batchFile) => {
...
main();
//# sourceMappingURL=movieComparison.js.map